...
public class RetailSelectionPartLightingTests
{
    ...
    public void APartFlash_lightsOnlyItsOwnParts()
    {
        ...
        Assert.True(pulse.HasPartLighting(Entity));
        foreach (int lit in new[] { 0x00, 0x09, 0x0A })
            Assert.True(pulse.TryGetPartLighting(Entity, lit, out _), $"part 0x{lit:X} should be lit");
        foreach (int dark in new[] { 0x01, 0x08, 0x0B, 0x10, 31 })
            Assert.False(pulse.TryGetPartLighting(Entity, dark, out _), $"part 0x{dark:X} should be dark");
    }
    ...
    public void APartFlash_isBrightThenDarkThenOver()
    {
        ...
        Assert.True(pulse.TryGetPartLighting(Entity, 0x10, out var bright));
        Assert.Equal(RetailSelectionLighting.High, bright);

        _now += Step;
        pulse.Tick();
        Assert.True(pulse.TryGetPartLighting(Entity, 0x10, out var dark));
        Assert.Equal(RetailSelectionLighting.Low, dark);

        _now += Step;
        pulse.Tick();
        Assert.False(pulse.HasPartLighting(Entity));
        Assert.False(pulse.TryGetPartLighting(Entity, 0x10, out _));
    }

    [Fact]
    public void APartFlash_doesNotReachAnotherEntity()
    {
        var pulse = New();
        pulse.StartParts(Guid, Entity, Mask(0x10));

        Assert.False(pulse.HasPartLighting(Entity + 1u));
        Assert.False(pulse.TryGetPartLighting(Entity + 1u, 0x10, out _));
    }
    ...
    public void AWorldPick_isUnchanged_wholeEntityAndFourFlips()
    {
        ...
        Assert.False(pulse.HasPartLighting(Entity));          // never per part
    ...
    public void APartFlashReplacesAWorldPick_andBackAgain()
    {
        ...
        Assert.False(pulse.TryGet(Guid, Entity, out _));
        Assert.True(pulse.TryGetPartLighting(Entity, 0x09, out _));

...
**Recover before giving up.** Planning again from the spot a body is pressed against rarely frees it, so a walk that stops making progress tries a different way off what it met at each stop (back, the roomiest spot nearby, a sidestep each way, a hop) before planning again. A walk ends blocked only after the whole ladder, so its caller gets an answer; a follow starts over instead, because bots play for hours. Keep-out spots are added only for the first stops, so a body stuck in one place does not ring itself in.
